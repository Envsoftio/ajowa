...
begin
  ...
  loop
    ...
    if v_transaction_id is null then
      ...
      update transactions
      set transaction_type = v_row.transaction_type::transaction_type,
          category_id = v_category_id,
          bank_account_id = null,
          title = concat(v_row.category_name, ' - ', v_row.month_label),
          description = v_description,
          counterparty_name = case when v_row.transaction_type = 'EXPENSE' then v_row.category_name else 'Imported collection summary' end,
          transaction_date = v_row.transaction_date,
          amount = v_row.amount,
          status = 'POSTED',
          approved_by_user_id = coalesce(approved_by_user_id, v_admin_user_id),
    ...
    if v_journal_entry_id is null then
      ...
      set voucher_number = v_journal_voucher,
          entry_date = v_row.transaction_date,
          description = v_description,
          status = 'DRAFT',
          updated_at = now()
    ...
    on conflict (journal_entry_id, line_no) do update
    set account_head_id = excluded.account_head_id,
  ...
  delete from society_bank_accounts
  where account_number = 'AJOWA-HISTORICAL-IMPORT'
    and ifsc_code = 'AJOWA000000';
