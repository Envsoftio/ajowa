...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const eligibleLookupMs = Math.round(performance.now() - startedAt)
    const enqueueStartedAt = performance.now()
    const notificationInput: Parameters<typeof enqueueDueBillingContactNotifications>[1] = {
    ...
    if (body.recipientUserIds?.length) {
      notificationInput.recipientUserIds = body.recipientUserIds
    }
    if (body.recipientRelationshipTypes?.length) {
      notificationInput.recipientRelationshipTypes = body.recipientRelationshipTypes
    }

    const queued = await enqueueDueBillingContactNotifications(client, notificationInput)
    ...
    const timingMs = {
      eligibleLookup: eligibleLookupMs,
      enqueue: enqueueMs,
      total: totalMs,
    }

    logger.info('Billing due reminders queued.', {
      requestedDueCount: body.dueIds.length,
      eligibleDueCount: dueIds.length,
      eventCount: queued.eventCount,
      audienceCount: queued.audienceCount,
